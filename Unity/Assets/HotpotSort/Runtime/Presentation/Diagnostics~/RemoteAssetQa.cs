...
static class RemoteAssetQa
{
    ...
    static async Task Main(string[] args)
    {
        ...
        await Run("R02-cold-warm-typed-readiness-and-immutable-input",async()=>{
            ...
            var packaged=RemoteAssetSource.Select("Packaged","https://ignored.invalid/","fixture-env",cloud.CloudFileId,"hotpot/release/hotpot-remote.bundle");
            using(var r=new Rig()){Check((await r.Build(source:packaged).PrepareAsync()).State==AssetReadiness.Ready&&r.Net.Calls==1&&r.Net.LastUri.OriginalString=="hotpot-local://package/hotpot/release/hotpot-remote.bundle"&&r.Fs.Files.Count==0,"packaged source bypasses network cache");}
