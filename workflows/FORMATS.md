...
Task Markdown 只允许一个 `harness-state` JSON 块。`contract` 是确认需求/QA Intent/接口/资产要求的来源。`runs`、`completed`、`requirements_confirmation`、`approvals`、`history` 由 `harness.py` 操作。追加 Notes 不属于合同，不改变 task_revision。
