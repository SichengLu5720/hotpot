 ...
 return async(event,context)=>{
  ...
  try{
   ...
   const result=await transaction(async tx=>{
    ...
    if(receipt){if(receipt.fingerprint!==fingerprint)fail('OperationConflict');return {snapshot:p,trade:receipt.trade};}
    let trade;
    if(action==='sync'){
     ...
     const tool=event.tool;if(!Number.isInteger(tool)||tool<0||tool>2||p.tools[tool]<1)fail('InsufficientInventory');p.tools[tool]--;p.appliedOperations.push(op);
    }else if(action==='create'){
     const target=text(event.targetAccount);if(!/^wx_[a-f0-9]{64}$/.test(target)||target===actor)fail('InvalidTarget');
     ...
     const requestId=text(event.tradeId),tradeKey='trade:'+requestId;if(await tx.get(tradeKey))fail('TradeIdConflict');
     trade={requestId,initiator:actor,recipient:target,offeredId:event.offeredId,receivedId:event.receivedId,status:'Pending',createdUtc:utc(time),expiresUtc:utc(time+86400000)};await tx.set(tradeKey,trade);
     for(const owner of [actor,target]){const index=await tx.get('index:'+owner)||{ids:[]};if(index.ids.length>=1024)fail('RequestLimit');index.ids.push(requestId);await tx.set('index:'+owner,index);}
     ...
     if(![trade.initiator,trade.recipient].includes(actor))fail('NotParticipant');
     ...
     if(action==='withdraw'?actor!==trade.initiator:actor!==trade.recipient)fail('NotAuthorized');
     if(trade.status!=='Pending')fail('TradeClosed');if(time>=Date.parse(trade.expiresUtc))fail('TradeExpired');
     if(action==='accept'){
      if(!verifyFriend)fail('FriendVerificationUnavailable');if(await verifyFriend(trade.initiator,actor,context)!==true)fail('NotFriends');
