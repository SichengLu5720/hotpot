...
(async()=>{
 ...
 await test('S07-malformed-oversized-enums-utc-outbox',async()=>{const r=rig();for(const mutate of [d=>d.settings={},d=>d.rewards=[award('bad','2026-02-30T00:00:00.0000000+00:00')],d=>d.rewards=[{...award('bad'),route:99}],d=>d.rewards=[{...award('bad'),rewardKind:5}],d=>d.rewards=[{...award('bad'),timeSource:2}],d=>d.rewards=[award('bad','2026-09-22T00:00:00.0000000+08:00')],d=>d.firstWinDays=['20260230'],d=>d.firstWinDays=['20260922','20260922'],d=>d.pending=[{operationId:'win:20260922',kind:'win',entityId:'20260922'}],d=>d.firstWinDays=Array(4097).fill('20260922'),d=>d.account='a'.repeat(p.MAX_BYTES+1)]){const d=document();mutate(d);assert.equal((await r.handle(envelope(d),context)).status,'Rejected');}assert.equal(r.db.writes,0);});
