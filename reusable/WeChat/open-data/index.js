'use strict';
const model = (() => { const module = {exports: {}};
'use strict';
// Own record only; friend records never leave the open data context.
const KEY = "harness_campaign_rank_v1";
function valid(v) {
...
// Inspect with an open-data debugger breakpoint; never log or export player data.
let lastDiagnostic = 'idle';
function diagnostic(code) { lastDiagnostic = code; }
class OperationFailure extends Error { constructor(code) { super(code); this.code = code; } }
function failure(code) { return new OperationFailure(code); }
function call(name, options) {
    const writing = name === 'setUserCloudStorage';
    const stage = writing ? 'write' : name === 'getUserCloudStorage' ? 'own_read' : 'friends_read';
    if (typeof wx[name] !== 'function') return Promise.reject(failure(stage + '_api_missing'));
    if (writing && pendingWrite) return Promise.reject(failure('write_unresolved'));
    const ticket = writing ? {record: JSON.parse(options.KVDataList[0].value)} : null;
    if (writing) pendingWrite = ticket;
    return new Promise((resolve, reject) => {
        let settled = false;
        const timer = setTimeout(() => {settled = true; reject(failure(stage + '_timeout'));}, 10000);
        const finish = (ok, result) => {
            ...
            if (ok) resolve(result); else reject(failure(stage + '_failed'));
        };
...
function sync() {
    ...
    if (!model.valid(candidate)) return Promise.reject(failure('local_record_invalid'));
    activeSync = (async () => {
        const response = await call('getUserCloudStorage', {keyList: [model.KEY]});
        let remote, merged;
        try { remote = model.parse(response && response.KVDataList); merged = model.merge(remote, candidate); }
        catch (_) { throw failure('own_record_invalid'); }
...
async function refresh() {
    ...
    timeout = setTimeout(() => { if (visible && request === generation) { diagnostic('refresh_timeout'); state = 'failure'; draw(); } }, 12000);
    try {
        await sync();
        const result = await call('getFriendCloudStorage', {keyList: [model.KEY]});
        if (!visible || request !== generation) return;
        try { data = model.snapshot(result && result.data, own); }
        catch (_) { throw failure('friends_response_invalid'); }
        state = data.empty ? 'empty' : 'ready';
        diagnostic(data.self ? 'refresh_' + state : 'self_identity_ambiguous');
        images.clear();
    ...
    } catch (error) { if (visible && request === generation) { diagnostic(error instanceof OperationFailure ? error.code : 'refresh_failed'); state = 'failure'; } }
...
function decodeMessage(raw) {
    ...
    if (typeof message === 'string') {
        try { message = JSON.parse(message); } catch (_) { return null; }
    }
    if (!message || typeof message !== 'object' || Array.isArray(message)) return null;
    const positive = value => Number.isFinite(value) && value > 0;
    if (message.type === 'WXRender') return Number.isFinite(message.x) && Number.isFinite(message.y) &&
        positive(message.width) && positive(message.height) && positive(message.devicePixelRatio) ? message : null;
    if (message.type === 'WXDestroy' || message.type === 'WXShow') return message;
    if (message.type !== "harness-friends-v1") return null;
    if (message.command === 'close') return message;
    if (message.command === 'layout') return positive(message.width) && positive(message.height) ? message : null;
    if (message.command === 'sync' || message.command === 'open') return model.valid(message.record) ? message : null;
    return null;
}
wx.onMessage(raw => {
    // Unity PostMessage forwards JsonUtility's string; SDK lifecycle events are objects.
    // Validate the complete command before changing any state or calling platform APIs.
    ...
    if (message.command === 'close') {visible = false; ++generation; clearTimeout(timeout); touch = null; return;}
    ...
    if (message.command === 'sync') {sync().catch(error => diagnostic(error instanceof OperationFailure ? error.code : 'sync_failed')); return;}
